...
if [ "$TESTFROMVER" == "$FROMVER" ]; then
	FROMVER=`echo $FROMVER | sed 's/-SNAPSHOT/.qualifier/g'`
fi

if [ "$TESTTOVER" == "$TOVER" ]; then
	TOVER=`echo $TOVER | sed 's/-SNAPSHOT/.qualifier/g'`
fi

perl -pi -e "s/$FROMVER/$TOVER/g" `find . -name MANIFEST.MF -or -name feature.xml`
